...
| R18 | A score panel SHALL display the current score (e.g. **"You N : M Robot"**) and update after every round. Its language follows the R35 toggle. | MUST | §1,§4,§7 | T16 |
| R19 | The **first** side to reach **3** points SHALL end the game and trigger a win/lose end state showing a win or lose banner (e.g. **"YOU WIN"** / **"YOU LOSE"**). Its language follows the R35 toggle. | MUST | §1,§2,§12 | T9,T17 |
| R20 | A **restart** SHALL begin a fresh game (new shuffled deck, scores 0, hands one-of-each). Offered both **automatically after the end state** and **on demand via the Restart button** (R33). | SHOULD | §2,§7 | T17 |
...
| R25 | Each resolved round SHALL play a short synthesized audio **stinger** via an `AudioStreamPlayer`, keyed to `outcome`: **win**, **lose**, or a neutral **draw** tone; the game-over end state plays win or lose. WAVs are generated by `tools/gen_sfx.py` (stdlib, no deps) → `art/{win,lose,draw}.wav`. | SHOULD | §7,§9 | T21 |
...
### 3.9 In-game control panel (buttons, language & music)

> Not a settings/onboarding menu (those stay out of scope, §8) — a fixed control strip, always
> visible, no navigation. The Language toggle re-languages **all** in-game text.

| ID | Requirement | Pri | Src | Verified |
|----|-------------|-----|-----|----------|
| R32 | A panel of **four retro/arcade-styled buttons — Restart, Rules, Track, Language** — SHALL be shown fixed at the player's **right side, at eyeline and within arm's reach** (turn right to face it), built as an **XR Tools `Viewport2Din3D`** 2D UI. Buttons SHALL be activatable by **fingertip poke (touch)** via an XR Tools `Poke` on each hand, and also by the controllers' **laser pointer** (`function_pointer`). The panel's readable face turns toward the player; buttons are large and vertically spread for easy touch. | MUST | §6 | T27 |
| R33 | The **Restart** button SHALL begin a fresh game (new shuffled deck, scores 0, hands one-of-each), re-deal the visible hand, AND **restore the player's start position** (the rig's authored offset) — even mid-round. | MUST | §2,§7 | T28 |
| R34 | The **Rules** button SHALL **toggle** a brief **how-to-play card** covering: the goal (first to 3), the beats cycle (Fish→Bird→Dino→Fish), how to grab a card (**pinch**), and where to throw it (the **red felt circle**). The card SHALL open **beside** the buttons (not over them) with **all text visible**, and a second Rules tap SHALL close it. | SHOULD | §1 | T29 |
| R35 | The **Language** toggle SHALL switch **all in-game text** (rules card, scoreline, win/lose banner, and every button caption) between **English and Japanese**. The toggle's own caption SHALL read **日本** in English mode and **EN** in Japanese mode. The scoreline and end banner SHALL show a **single** language per selection (not both at once). | MUST | §6 | T30 |
| R36 | Looping **background music** SHALL play from game start, beginning on the **retro-gaming** track. The music control SHALL be a **split button**: a **play/pause** half (icon reflects current state) and a **skip** half that advances to the **next** track in a fixed cycle; on a skip the new track's **name** SHALL appear for **~2 s** beside the button. | SHOULD | §6 | T31 |
| R37 | When a win/lose/draw **jingle** plays, the background music SHALL **pause (duck)** for the jingle and **resume** when it finishes. | SHOULD | §7 | T32 |
...
- **Control panel:** chunky **retro/arcade buttons** (dark slab, square corners, bright bevel that
  visibly depresses on press, pixel `DotGothic16` font, dark-on-bright invert when targeted),
  matching the cards' pixel aesthetic (R32).
...
- **ScorePanel** (`Label3D`) — shows the scoreline (R18); switches to the win/lose banner on game
  over (R19); language follows the toggle (R35).
- **Control panel** (`Viewport2Din3D` → `Hud.tscn`) — at the player's right, eyeline, in reach; retro
  buttons (Restart / Rules / play-pause + skip / Language) + a side Rules card + a transient track-name
  popup, pressed by fingertip poke or the `function_pointer` laser (R32–R37). The `Lang` autoload holds the
  one language flag the whole UI reads; the `Music` autoload owns the looping soundtrack + track cycle +
  play/pause + jingle duck.
...
5. Score panel updates (R18) and the round's win/lose/draw stinger plays (R25).
...
**settings menus / best-of-N config / multi-screen onboarding flows** · more than 3 character types
or any deckbuilding meta · **miss handling** · sound design beyond the per-round win/lose/draw
stingers (R25) and the **selectable looping background music** (R36–R37) — no ambience or
spatialized/positional audio.

> The fixed three-button **control panel** (Restart / Rules / Language, §3.9) is **in scope** and is
> *not* a "menu" in the above sense — no navigation, no settings tree, no config. The Rules overlay
> is a one-screen reference card, not an interactive tutorial.
...
(run before any VR — DESIGN §5); T10–T20, **T24** (room shell, R29), **T27–T32** (control panel + music),
and **T21** (round audio, R25) are **on-device** (Quest 3); **T22–T23 and T25–T26 cover stretch**
(T25/T26 only if the wall animation R30/R31 is built).
...
| T21 | Round audio + juice | Each round plays its win/lose/draw stinger (game-over plays win/lose); `python tools/gen_sfx.py selftest` passes. If built, a win also glows/haptics once (R26). | R25,R26 |
...
| T27 | Control panel present & clickable | In headset, turn right to the eyeline panel; point at it. → Three retro buttons visible and within reach; the laser highlights a button and trigger-click presses it (visibly depresses). | R32 |
| T28 | Restart button | Press Restart mid-game and at the end state. → Scores reset to 0, fresh hand dealt, view recentered; no stuck/double-dealt round if pressed mid-resolution. | R20,R33 |
| T29 | Rules overlay | Press Rules. → A how-to overlay appears (goal, beats cycle, pinch-to-grab, throw on red felt); press again/close to dismiss. | R34 |
| T30 | Language toggle | Press the 日本/EN toggle. → Every caption, the rules text, the scoreline, and the win/lose banner switch language together; the toggle reads 日本 in English, EN in Japanese; scoreline shows one language only. | R35 |
| T31 | Music split button | Launch (music plays, retro-gaming). Press play/pause → music pauses/resumes and the icon flips. Press skip → next track, its name shows ~2 s, repeated skips cycle all tracks. | R36 |
| T32 | Music ducks for jingle | Win/lose/draw a round. → The music pauses while the jingle plays and resumes after it ends. | R37 |
...
| R10 | T2 | R20 | T17,T28 | NFR4 | T10 |
| R29 | T24 | R30 | T24,T25 | R31 | T25,T26 |
| R32 | T27 | R33 | T28 | R34 | T29 |
| R35 | T30 | R36 | T31 | R37 | T32 |
...
- **Restart (R20/R33)** = the **Restart button** on the control panel (and an auto-restart after the
  end state) → `PlayZone.restart()`: `GameState.new_game()`, `GameRoot` respawns hands, robot face
  reset, and (button only) `XRServer.center_on_hmd`. A `_gen` counter makes a mid-round restart cancel
  the in-flight round's awaits. *(Superseded the earlier "grabbable play-again card" idea — a button
  is one place and also serves mid-game restarts.)*
- **Control panel + language (R32–R35)** = an XR Tools **`Viewport2Din3D`** rendering a code-built 2D
  retro UI (`game/Hud.gd`/`Hud.tscn`), pressed by an XR Tools **`Poke`** on each hand (fingertip touch)
  or the **`function_pointer`** laser. i18n is one tiny autoload (`game/Lang.gd`: a `jp` flag +
  `changed` signal + `t(en, ja)` helper) — every label is a `Lang.t(...)` call, so the toggle
  re-languages the whole game from one place. No translation tables, no locale files.
- **Background music (R36–R37)** = one autoload, `game/Music.gd`: a looping `AudioStreamPlayer` over the
  `music/` tracks, opening on retro-gaming. The HUD music control is a **split button** — `toggle_play()`
  (left half, play/pause icon) and `next_track()` (right half, skip → returns the name for the HUD's ~2 s
  popup). `duck()`/`resume()` pause it around a jingle (PlayZone on stinger play / `_sfx.finished`).
  Manual pause and jingle-duck are tracked as **separate flags** (`_user_paused` / `_ducked`) composed in
  `_apply_pause()`, so a jingle's resume can't undo a manual pause. Track list is the whole config.
- **Panel renders mirrored → flip content, not facing.** The `Viewport2Din3D` showed mirrored text from
  *both* Y-facings tried, so `main.tscn`'s `Hud3D` **negates its X basis** (left-handed transform =
  horizontal mirror). Input still maps right — the addon's body uses the collider's `affine_inverse()`.
